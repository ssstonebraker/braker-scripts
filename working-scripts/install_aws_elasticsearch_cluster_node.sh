#!/bin/bash
#
# Name: install_elasticsearch_cluster_node.sh
# Author: Steve Stonebraker
# Date: 9/3/2013
# Purpose: Install elastic search 0.20.6 on Ubuntu 12.04 LTS for deployment in an AWS Cluster
...
read -p "Path on disk (no trailing slash) where you want to store data: " edata_location
...
echo "discovery.ec2.tag.elastic-cluster: ${aws_tag}" >> ${ECONFIG}
echo "path.data: ${edata_location}" >> ${ECONFIG}

if [ -d "/opt/elasticsearch" ]; then
    chown -R elasticsearch:elasticsearch /opt/elasticsearch
fi

if [ -d ${edata_location} ]; then
    chown -R elasticsearch:elasticsearch ${edata_location}
fi
